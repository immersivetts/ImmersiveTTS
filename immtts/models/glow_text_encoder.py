...
class GlowTextEncoder(BaseModule):
    def __init__(
        self,
        n_vocab,
        n_feats,
        ...
        p_dropout,
        window_size=None,
        pre_spk_emb_dim=512, # 64
        spk_emb_dim=192,
        n_spks=1,
    ):
        super(GlowTextEncoder, self).__init__()
        ...
        self.proj_spk = torch.nn.Linear(pre_spk_emb_dim, spk_emb_dim)
    ...
    def forward(self, x, x_lengths, spk=None):
        ...
        if self.n_spks > 1:
            spk_emb = self.proj_spk(spk)
            x = torch.cat([x, spk_emb.unsqueeze(-1).repeat(1, 1, x.shape[-1])], dim=1)
